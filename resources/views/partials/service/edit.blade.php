<v-dialog v-model="editServiceModal">
    ...
    <v-card class="service_modal">
        ...
        <v-card-text>
            <v-card>
                ...
                <v-card-text>
                    <v-layout row wrap>
                        <v-flex md6>
                            <v-text-field v-model="eService.name" label="Service Name"></v-text-field>
                            <v-text-field v-model="eService.fee" label="Fee"></v-text-field>
                            <v-textarea v-model="eService.description" label="Service Description"></v-textarea>
                            <v-select v-model="eService.category_id" label="Category" :items="categories" item-text="name" item-value="id"></v-select>
                            <v-checkbox v-model="eService.show_on_booking_form" label="Show on online booking form"></v-checkbox>
                        </v-flex>
                        <v-flex md-6>
                ...
                <v-card-text>
                    <v-layout row wrap>
                        <v-flex xs6 sm3 md3>
                            <v-text-field
                                label="Hours"
                                v-model="eService.duration_hours"
                            ></v-text-field>
                        </v-flex>
                        <v-flex xs6 sm3 md3>
                            <v-text-field
                                label="Minutes"
                                v-model="eService.duration_minutes"
                            ></v-text-field>
                        </v-flex>
                    </v-layout>
                </v-card-text>
        ...
        <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" flat @click="editServiceModal = false">
            Cancel
        </v-btn>

        <v-btn color="primary" flat @click="updateService">
            Save
        </v-btn>
        </v-card-actions>
    </v-card>
